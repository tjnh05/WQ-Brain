...
- **数据集深度分析技术**:
    ...
    - 搜索空间压缩技术（10万倍缩减）
- **操作符组合优化**:
    - Tail家族操作符与rank函数组合策略
    - 参数设置的最佳实践
    - 操作符相互替换规则
    - 经济学逻辑验证方法
- **表达式验证与质量控制**:
    - 语法验证器集成（支持所有WorldQuant Brain操作符）
    - 无效表达式早期识别机制
    - 回测资源优化策略
- **区域特定优化策略**:
    - IND区域: Model/Analyst/Option优先，Market中性化最佳
...
3. **[CRITICAL]**: 查阅 **`HowToUseAllDatasets`** 和 **`HowToUseAIDatasets`** 文件夹中相关文档。
    - **数据集深度分析**: 
        - OS表现评估: 优先选择OS表现优异的数据集
        - 字段分类分析: 使用AI对字段进行经济学维度分类
        - 相关性矩阵构建: 分析字段间相关性，识别代表性字段
        - 搜索空间压缩: 基于分类结果压缩搜索空间（10万倍缩减技术）
...
2. **数据集选择与AI辅助分析**:
    - **数据集优先级策略**:
        - **PRIORITY 1**: 基于OS表现选择数据集，优先选择OS表现优异的数据集
        - **区域特定策略**: IND区域优先Model/Analyst/Option，USA/EUR区域根据具体情况调整
        - **数据增量考虑**: 优先选择新增数据量大的区域
    - **AI辅助字段分析**:
        - **字段分类**: 使用AI对数据集字段进行经济学维度分类
        - **相关性分析**: 构建字段间相关性矩阵，识别代表性字段
        - **模板推荐**: 基于分类结果推荐有经济学意义的模板
        - **搜索空间优化**: 应用10万倍缩减技术
    ...
    - **实战经验库**: 参考`HowToUseAIDatasets`中的成功案例和失败模式
3. **智能改进策略**:
